...
| Ref-only push validation | Existing connectivity walk for every non-deletion tip over local published packs plus this upload; local hard links only, no additional bucket requests | 0 extra; rejected targets never reach log/manifest publication | `smart.rs::receive_pack_process` |
| Compaction publish | lease acquire (1 GET → 1 CAS put), then pack/idx PUT ∥ log PUT → manifest CAS; lease renewal is only on a delayed/long-running path near expiry (no extra happy-path round trip) | — | `ops.rs::compact_repo`, `publish.rs::publish_compact_impl` |
...
| Maintainer bundle pass, refs level (retention + settle closed slots) | 1 list GET → at most 1 **fenced** CAS (retention) → at most **1 fenced CAS for every verdict batch** (`record_skipped_many`; was one CAS per settled slot until 2026-08-22: a rig repo with 9,654 closed slots paid 9,654 CAS per pass and, past the 4,096-verdict cap, forever). A fenced mutation is list-lease GET→CAS, list-fence GET→CAS, logical list GET→CAS, release CAS: 7 requests / 7 sequential rounds on the write path (6 when the closure is a no-op); this is paid only when the list changes, while a no-op retention pass remains one GET. | ≤ 3 read rounds; +7 write rounds | `walgit-bundle/src/lib.rs::settle_closed_slots`, `ops.rs::record_skipped_many`, `ops.rs::cas_update_list` |
