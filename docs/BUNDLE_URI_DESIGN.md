...
| **Naming / storage** | `bundles/<strategy>/<slotUTC>-<sha1 of content>.bundle` (immutable, content-addressed, ETag = checksum), list at `bundles/list.pb` (CAS). Keys never overwritten. The list carries coordination-only fencing metadata: a shared list-writer token plus the per-strategy lease token used for a publish. Every list mutation acquires `leases/bundle-list.pb`, advances the list token before the final CAS, and releases it with a conditional tombstone. | Immutable → `Cache-Control: immutable`, Range, CDN; CAS'd list = atomic publish. Git ignores the extra protobuf fields. A crashed list writer is recoverable after the 30 s list-lease TTL; a strategy writer cannot publish after a successor has advanced its token. |
